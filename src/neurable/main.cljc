(ns neurable.main
  "Kotodama WASM entrypoint for the Neurable clean-room actor (L4) — Clojure port.
  ...
  facts (`neurable.<Entity>/<field>`); `*store*` is the in-memory materialization
  used by the contract test and by the WASM runtime before a live engine binds."
  (:require [clojure.string :as str]))

(def ns-prefix "neurable")
(def tier "L4")
...
(def entity-specs
  [{:entity "Device"      :plural "devices"      :id-prefix "neurable_dev"
    :fields [:model :channels :samplingHz :firmware] :required [:model :channels]
    :coerce {:channels :int :samplingHz :int} :refs {}}
   {:entity "Session"     :plural "sessions"     :id-prefix "neurable_ses"
...
(defn emit-facts
  "EAVT facts for one record: {\"neurable.<Entity>/<field>\" v ...}. The datomic
...
(defn healthz [] [{:status "ok" :actor "neurable-compat" :tier tier :entities entities} 200])
...
(defn start! [] :neurable-compat/ready)
